...
# ibm-granite/granite-4.1-8b on OpenShift with one DGX H100 (8 H100 SXM GPUs).
#
# Usage:
...
#
# Env vars:
#   NAMESPACE                (default: llm-d-granite-approx-kv)
...
#                            HF_TOKEN env is unset (default: llm-d-precise-prefix-granite-approx)

set -euo pipefail

NAMESPACE="${NAMESPACE:-llm-d-granite-approx-kv}"
SOURCE_NAMESPACE="${SOURCE_NAMESPACE:-llm-d-precise-prefix-granite-approx}"
...
EPP_RELEASE="precise-granite-approx"
...
run_test() {
    ...
    local body
    body=$(jq -n --arg p "$prompt" '{model:"ibm-granite/granite-4.1-8b",prompt:$p,max_tokens:10}')
